...
func (srv *Server) uploadServerCertificate(w http.ResponseWriter, req *http.Request, reqId string) (interface{}, error) {
	privateKey := req.FormValue("PrivateKey")
	if !strings.Contains(privateKey, "-----BEGIN RSA PRIVATE KEY-----") {
		return nil, &iam.Error{
			StatusCode: 400,
			Code:       "BadRequest",
			Message:    "MalformedCertificate: Unable to parse key.",
		}
	}

	certName := req.FormValue("ServerCertificateName")
	if len(certName) > 128 {
		return nil, &iam.Error{
			StatusCode: 400,
			Code:       "ValidationError",
			Message:    fmt.Sprintf("1 validation error detected: Value '%s' at 'serverCertificateName' failed to satisfy constraint: Member must have length less than or equal to 128", certName),
		}
	}

	if srv.certificates == nil {
		srv.certificates = map[string]Certificate{}
	}

	if _, ok := srv.certificates[certName]; ok {
		return nil, &iam.Error{
			StatusCode: 409,
			Code:       "EntityAlreadyExists",
			Message:    fmt.Sprintf("The Server Certificate with name %s already exists.", certName),
		}
	}

	srv.certificates[certName] = Certificate{
		Body:  req.FormValue("CertificateBody"),
		Chain: req.FormValue("CertificateChain"),
	}

	resp := iam.UploadServerCertificateResponse{
		ServerCertificateMetadata: iam.ServerCertificateMetadata{
			Arn:                   "arn:aws:iam::196214184981:server-certificate/" + certName,
			ServerCertificateId:   "fake-cert-id",
...
func (srv *Server) deleteServerCertificate(w http.ResponseWriter, req *http.Request, reqId string) (interface{}, error) {
	certName := req.FormValue("ServerCertificateName")
	delete(srv.certificates, certName)
	resp := &iam.SimpleResp{}
	return resp, nil
}

type GetServerCertificateResponse struct {
	ServerCertificateMetadata iam.ServerCertificateMetadata `xml:"GetServerCertificateResult>ServerCertificate>ServerCertificateMetadata"`
	ServerCertificateBody     string                        `xml:"GetServerCertificateResult>ServerCertificate>CertificateBody"`
	ServerCertificateChain    string                        `xml:"GetServerCertificateResult>ServerCertificate>CertificateChain"`
	RequestId                 string                        `xml:"ResponseMetadata>RequestId"`
}

func (srv *Server) getServerCertificate(w http.ResponseWriter, req *http.Request, reqId string) (interface{}, error) {
	certificate, ok := srv.certificates[req.FormValue("ServerCertificateName")]
	if !ok {
		return nil, &iam.Error{
			StatusCode: 404,
			Code:       "NoSuchEntity",
			Message:    fmt.Sprintf("cert not found"),
		}
	}
	resp := &GetServerCertificateResponse{ServerCertificateBody: certificate.Body, ServerCertificateChain: certificate.Chain}
	return resp, nil
}
...
var actions = map[string]func(*Server, http.ResponseWriter, *http.Request, string) (interface{}, error){
	...
	"UploadServerCertificate": (*Server).uploadServerCertificate,
	"DeleteServerCertificate": (*Server).deleteServerCertificate,
	"GetServerCertificate":    (*Server).getServerCertificate,
}
